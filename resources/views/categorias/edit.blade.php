@extends('layouts.app')
@section('title','Editar categoría — Spendly')
...
@section('content')
    ...
    <div class="cats-wrap">
        ...
        <div class="cats-topbar">
            ...
            <div class="text-end">
                <div class="cats-title">Editar categoría</div>
                <div class="cats-sub">
                    <span class="pill-tipo">
                        <i class="fa-solid {{ $categoria->tipo === 'gasto' ? 'fa-arrow-trend-down' : 'fa-arrow-trend-up' }}"></i>
        ...
        <section class="panel">
            ...
            <form method="POST" action="{{ route('categorias.update', $categoria) }}">
                ...
                <div class="mb-3">
                    ...
                    @php
                        $tipoValue = old('tipo', $categoria->tipo);
                    @endphp
                    <div class="d-flex gap-3">
                        <div class="form-check">
                            <input class="form-check-input"
                                   ...
                                   id="tipo_gasto"
                                   value="gasto"
                                {{ $tipoValue === 'gasto' ? 'checked' : '' }}>
                            <label class="form-check-label" for="tipo_gasto">
                                <i class="fa-solid fa-arrow-trend-down me-1"></i> Gasto
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input"
                                   ...
                                   id="tipo_ingreso"
                                   value="ingreso"
                                {{ $tipoValue === 'ingreso' ? 'checked' : '' }}>
                            <label class="form-check-label" for="tipo_ingreso">
                                <i class="fa-solid fa-arrow-trend-up me-1"></i> Ingreso
                            </label>
                        </div>
                    </div>
                    @error('tipo')
                ...
                <div class="mb-3">
                    ...
                    @else
                        <div class="form-text">Debe ser único dentro de tus {{ $tipoValue }}s.</div>
                        @enderror
                </div>
                ...
                @php
                    $iconDefault   = old('icon', $categoria->icon ?? 'fa-wallet');
                    $defaultColor  = old('color_hex', $categoria->color_hex ?? '#6C63FF');
                    $activaChecked = old('activa', $categoria->activa) ? true : false;
                @endphp
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center gap-3 mb-2">
                        <div id="symbolPreview"
                             ...
                             style="background: {{ $defaultColor }}; width:48px; height:48px;">
                ...
                <div class="form-check form-switch mb-4">
                    <input class="form-check-input"
                           ...
                           name="activa"
                           value="1"
                        {{ $activaChecked ? 'checked' : '' }}>
                    <label class="form-check-label" for="activa">Categoría activa</label>
                    <div class="form-text">Puedes desactivarla sin borrarla definitivamente.</div>
                </div>
